...
import { getMemberAddress, delAddress } from '@/services/address'
...
const deladdress = async (id: string) => {
  await delAddress(id)
  loadMemberAddress()
  uni.showToast({ icon: 'error', title: '删除成功' })
}
          ...
          >
            <view class="item">
              <view class="user">
                ...
                <text :class="{ badge: item.isDefault === 1 }"> 默认 </text>
              </view>
              <view class="locate">
                {{ item.fullLocation }}{{ item.address }}
              </view>
              <navigator
                :url="`./form?id=${item.id}&type=${index}`"
                class="edit"
                hover-class="none"
              >
                修改
            ...
            <template v-slot:right>
              <view class="swipe-cell-action">
                <button class="delete-button" @click="deladdress(item.id)">
                  删除
                </button>
              </view>
            </template>
