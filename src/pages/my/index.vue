...
const { profile, isLogin, showAvatar, showNickname } = storeToRefs(Member)
// 定义处理头像信息计算属性
...
<template>
  <scroll-view scroll-y enhanced :show-scrollbar="false" id="scrollView">
    <view class="viewport" :style="{ paddingTop: safeArea!.top + 40 + 'px' }">
      <!-- 个人资料 -->
      <view class="profile">
        <view class="overview" @tap="goProfile">
          <image class="avatar" :src="showAvatar" mode="aspectFill"></image>
          <view class="meta">
            <!-- 用户昵称 -->
            <view class="nickname">{{ showNickname }}</view>
            <view class="extra">
              <!-- 登录状态 -->
              ...
              <!-- 未登录状态 -->
              <text class="tips" v-else>点击登录账号</text>
            </view>
          </view>
        </view>
